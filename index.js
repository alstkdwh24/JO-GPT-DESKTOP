    ...
    app.whenReady().then(() => {
        ...
        if (process.defaultApp) {
            if (process.argv.length >= 2) {
                app.setAsDefaultProtocolClient(CUSTOM_SCHEME, process.execPath, [path.resolve(process.argv[1])]);
            }
        } else {
...
function handleCustomProtocol(url) {
    ...
    try {
        ...
        if (token) {
            ...
            console.log("인증 에러 감지:", error);
            // 에러가 포함된 index.html 로드
            mainWindow.loadFile('index.html', { query: { error: error } });
        } else {
...
ipcMain.on('clear-session', async (event) => {
    try {
        ...
        console.log("세션 데이터가 완전히 삭제되었습니다.");

      // 2. 앱을 끄지 않고, 처리가 끝났음을 화면에 알림
